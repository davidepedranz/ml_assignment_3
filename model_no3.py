...
from nn_utilities import convolutional_layer, softmax_layer, cross_entropy_fun, accuracy_fun
...
def main():
    ...
    layer_2 = convolutional_layer(layer_1, [5, 5, 16], [32], '2_convolutional')
    layer_2_flat = tf.reshape(layer_2, [-1, 7 * 7 * 32], 'flatten')

    # 3th layer was removed
    # 4th layer: Readout Layer
    y_predicted_one_hot, y_predicted_label = softmax_layer(layer_2_flat, [7 * 7 * 32], [10], '4_softmax')
    ...
    with sess.as_default():
        ...
        for i in range(1, N_EPOCHS + 1):
            ...
            if i % SUMMARY_EVERY == 0:
                ...
                summary_train = sess.run(merged, feed_dict={x: train_images, y_: train_labels})
                train_writer.add_summary(summary_train, i)
                summary_test = sess.run(merged, feed_dict={x: mnist.test.images, y_: mnist.test.labels})
                test_writer.add_summary(summary_test, i)

                # save accuracy for plot
                train_acc = sess.run(accuracy, feed_dict={x: train_images, y_: train_labels})
